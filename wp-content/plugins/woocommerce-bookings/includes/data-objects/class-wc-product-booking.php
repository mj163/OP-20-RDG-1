	...
	*/
	...
	public function get_blocks_availability( $start_date, $end_date, $qty, $booking_resource = null ) {
		...
		if ( in_array( $this->get_duration_unit(), array( 'minute', 'hour' ) ) ) {
			...
			$existing_bookings = WC_Bookings_Controller::get_bookings_in_date_range( $start_date, $end_date, $this->has_resources() && $resource_id ? $resource_id : $this->get_id() );
		} else {
			$existing_bookings = WC_Bookings_Controller::get_bookings_in_date_range( $start_date, $end_date, $this->has_resources() && $resource_id ? $resource_id : $this->get_id() );
	...
	public function get_blocks_in_range_for_hour_or_minutes( $start_date, $end_date, $intervals, $resource_id, $booked ) {
		...
		$buffered_minutes   = $this->get_buffered_minutes( $minutes_booked );
		...
		while ( $check_date <= $end_date ) {
			...
			if ( ! empty( $buffered_minutes ) ) {
				$bookable_minutes_for_date  = $this->apply_buffers( $bookable_minutes_for_date, $check_date, $buffered_minutes );
			}
	...
	/**
	 * @since 1.9.13
	 * @version 1.10.0 re purposed to get and existing buffer period and the minutes bookable.
	 *                 then applying buffered to those minutes.
	 *
	 * @param array $bookable_minutes_for_date
	 * @param integer $check_date
	 * @param array $buffered_timestamps
	 *
	 * @return mixed
	 */
	protected function apply_buffers( $bookable_minutes_for_date, $check_date, $buffered_timestamps ) {
		if ( empty( $buffered_timestamps ) ) {
			return $bookable_minutes_for_date;
		}

		// For $check_date, the hours start from "First block starts at". So if it's 8 am, $check_date will start at 8 am hours.
		// For $bookable_minutes_for_date, the first entry is 480 mins (which is 8 am).
		// So with wc_booking_minute_to_time_stamp, for every entry in bookable minutes for date, it adds the buffered timestamps.
		// In doing this addition, "First block starts at" is calculated twice, so we get 16:00 to be the earliest possible bookable period.
		// To fix this, we need to reset the hours for $check_date at the beginning of the day.
		$check_date = strtotime( date( 'Y-m-d', $check_date ) );

		$bookable_time_stamps = array_map( 'wc_booking_minute_to_time_stamp', $bookable_minutes_for_date, array_fill( 0, count( $bookable_minutes_for_date ), $check_date ) );
		$minutes_after_applied_buffer = array_diff( array_values( $bookable_time_stamps ), array_keys( $buffered_timestamps ) );
		return array_map( 'wc_booking_time_stamp_to_minutes_after_midnight', $minutes_after_applied_buffer );
	}

	/**
	 * From the minutes not available determine the minutes
	 * that is buffered and return them.
	 *
	 * @since 1.10.0 renamed from apply_buffer to _get_buffered_minutes and made protected.
	 *
	 * @param array $not_available_minutes
	 *
	 * @return array $buffer_periods_in_timestamps
	 */
	protected function get_buffered_minutes( $not_available_minutes ) {
		$buffer_period_minute_count   = $this->get_buffer_period_minutes();
		...
		foreach ( $not_available_minutes as $timestamp => $bookings_count ) {
			...
			if ( ! isset( $not_available_minutes[ $next_minute ] ) ) {
				for ( $j = 0;  $j < $buffer_period_minute_count; $j++ ) {
					// bookings count represents number of bookings per slot, the same
					// should be buffered.
					$buffer_periods_in_timestamps[ $next_minute ] = $bookings_count;
			...
			if ( $this->get_apply_adjacent_buffer() && ! isset( $not_available_minutes[ $previous_minute ] ) ) {
				for ( $j = 0;  $j < $buffer_period_minute_count; $j++ ) {
		...
		return $buffer_periods_in_timestamps;
	}
